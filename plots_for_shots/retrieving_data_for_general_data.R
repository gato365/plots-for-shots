...
## Creation of heatmap_df
sum_df = data.frame()
for(season_num in 1:15){
  nba_data = nba %>%
    filter(Season == paste0("season_",season_num,sep=""))%>%
    select(-Rk,-G,-Date,-Age,-Tm,-Opp,-GS,-MP,-X3P_Percent,
           -GmSc,-FT_Percent,-FGA,-X3PA,-FTA,-FG_Percent,-Season,
           -Age_Years,-Age_Days,-Point_Margin,-Game_Location,-Game_Outcome,-number_game)
  ...
  summarize_data_nba = inner_join(x = summarize_data_nba_categ,
                                  ...
                                  by = c("Name")) %>%
    mutate(season_number = season_num)
  
  sum_df = bind_rows(sum_df,summarize_data_nba)
}

write.csv(sum_df,'heat_map_df.csv',row.names = F)
